...
class defensiveDQN(CaptureAgent):
  """
  DQN agent
  """
  ...
  def get_teammate_index(self,gameState):
    team = CaptureAgent.getTeam(self,gameState)
    teammate = int([i for i in team if i != self.index][0]) # get my teammate
    return  teammate


  def get_restart_position(self,gameState):
    #!! TODO fix this
    red = False
    if red:
      x=2 #! fix
    else:
      teammate = self.get_teammate_index(gameState)
  ...
  def updateLastReward(self, gameState):
    ...
    if (gameState.isOver()):
      ...
      if self.first_state:  # since we will start in the starting position duh
        ...
        if gameState.getAgentPosition(self.index) == gameState.getInitialAgentPosition(self.index):
          self.atCenter=False #need to get back to center
          return -100  # we were eaten and spawned back to start
